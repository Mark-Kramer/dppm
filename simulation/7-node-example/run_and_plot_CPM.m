function run_and_plot_CPM(nets, xy, clr, location, k_CPM)
  ...
  for repeats=1:100
      clear track
      [track.vertices, track.communities] = cpm(nets, k_CPM);
  ...
  communities = communities(isfinite(communities)); % Exclude nodes with no community assignment over all time (NaN).
  ...
  text(-3.5, 3, ['CPM(' num2str(k_CPM) '), N=' num2str(n_integer_communities)], 'FontSize', 12)
